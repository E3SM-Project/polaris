...
class Combine(Step):
    ...
    def __init__(self, component):
        ...
        filepath = os.path.join(self.SUBDIR, config_filename)
        config = PolarisConfigParser(filepath=filepath)
        config.add_from_package('polaris.topo', 'combine.cfg')
    ...
    def run(self):
        ...
        self._remap_global(out_filename=global_remapped_filename)
    ...
    def _create_global_tile(self, lon_tile, lat_tile):
        """
        ...
        # Load dataset
        ds = xr.open_dataset(f'{global_name}_cf.nc')
    ...
    def _remap_global(self, out_filename):
        """
        ...
        for lat_tile in range(lat_tiles):
            for lon_tile in range(lon_tiles):
                ...
                self._create_global_tile(lon_tile, lat_tile)
