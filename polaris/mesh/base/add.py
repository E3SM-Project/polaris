...
def add_spherical_base_mesh_step(
    ...
    resolution to a component (if one has not already been added)
    ...
    (
        ...
        config_filename,
        max_res,
    ) = _get_base_mesh_info(prefix=prefix, min_res=min_res, max_res=max_res)
    config = _get_base_mesh_config(
    ...
    kwargs = {
        ...
        'config': config,
        'config_filename': config_filename,
        'name': f'{prefix.lower()}_base_mesh_{res_str}',
        'mesh_name': mesh_name,
    }
    if prefix in ['icos', 'qu']:
        kwargs['cell_width'] = min_res

    base_mesh = mesh_component.get_or_create_shared_step(**kwargs)

    return base_mesh, res_str


def _get_base_mesh_info(prefix, min_res, max_res=None):
    if max_res is None:
    ...
    config_filename = f'{prefix_lower}_base_mesh_{res_str}.cfg'
    return res_str, mesh_name, subdir, config_filename, max_res


def _get_base_mesh_config(
    component,
    prefix,
    min_res,
    max_res,
