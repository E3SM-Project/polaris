...
class Init(Step):
    ...
    def _compute_init_topo(self):
        ...
        ocean_density = config.getfloat('isomip_plus', 'ocean_density')
        ...
        floating_mask = np.logical_and(
            land_ice_mask, ds_init.landIceFloatingFraction > 0
        )

        ds_init['landIceMask'] = land_ice_mask.astype(int)
        ds_init['landIceFloatingMask'] = floating_mask.astype(int)

        ds_init['landIceFraction'] = xr.where(
            land_ice_mask, ds_init.landIceFraction, 0.0
        )
        ds_init['landIceFloatingFraction'] = xr.where(
            land_ice_mask, ds_init.landIceFloatingFraction, 0.0
        )
        ...
        thin_film_mask = np.logical_and(
            land_ice_mask, np.logical_not(floating_mask)
        )
        ...
        if thin_film:
            ...
            ds_init['bottomDepth'] = xr.where(
                thin_film_mask, 0.0, ds_init.bottomDepth
            )
            ds_init['ssh'] = xr.where(thin_film_mask, 0.0, ds_init.ssh)

        # Deepen the bottom depth to maintain the minimum water-column
        # thickness
        min_column_thickness = max(
            min_column_thickness, min_levels * min_layer_thickness
        )
        min_depth = -ds_init.ssh + min_column_thickness
        too_thin = np.logical_and(
            active_ocean, ds_init.bottomDepth < min_depth
        )
        ds_init['bottomDepth'] = xr.where(
            too_thin, min_depth, ds_init.bottomDepth
        )
    ...
    def _compute_t_s_vel_coriolis(self):
        ...
        if not thin_film:
            ...
            ds_init['maxLevelCell'] = ds_init.maxLevelCell.where(
                active_ocean, 0
            )

        max_bottom_depth = -config.getfloat('vertical_grid', 'bottom_depth')
        frac = (0.0 - ds_init.zMid) / (0.0 - max_bottom_depth)
        ...
        ds_init['normalVelocity'] = normalVelocity.expand_dims(
            dim='Time', axis=0
        )
    ...
    def _plot(self):
        ...
        plot_horiz_field(
            ds_mesh=ds_mesh,
            field=ds['landIceFloatingFraction'],
        ...
        plot_horiz_field(
            ds_mesh=ds_mesh,
            field=ds['landIceGroundedFraction'],
