"""
Base class for steps that initialize the z-tilde vertical coordinate by
...
class ZTildeInitStep(Step, ABC):
    ...
    optionally override :py:meth:`_build_vert_coord_ds` when per-cell
    ...
    def init_tracers(
        ...
        at z-tilde layer midpoints for the current outer iteration.

        Parameters
        ----------
        ds : xarray.Dataset
            Current z-tilde dataset, including ``ZTildeMid``,
    ...
    def _build_vert_coord_ds(
        ...
        :py:func:`~polaris.ocean.vertical.ztilde.init_z_tilde_vertical_coord`
        ...
        -------
        xarray.Dataset
            Dataset with z-tilde coordinate variables added.
        """
        config = self.config
        ds = ds_mesh.copy()
    ...
    def run_z_tilde_init(
        ...
        -------
        xarray.Dataset
            Dataset containing all base-class output variables (see design
            doc Table 1 and Table 2) with ``long_name`` and ``units``
            attributes set.
        """
        logger = self.logger
        config = self.config
        ...
        for iteration in range(pseudothickness_iter_count):
            ...
            # Step 9: full-cell stagnation check — convergence check above
            # takes priority so that a perfect initial guess (scaling = 1)
            # exits cleanly rather than triggering this warning.
            if (
            ...
            # Step 10: proportional-ratio update
            scaling_factor = (
