...
class Init(ZTildeInitStep, OceanIOStep):
    ...
    x : numpy.ndarray
        ...
        :py:meth:`init_tracers` and :py:meth:`_build_vert_coord_ds`.
    """
    ...
    def _build_vert_coord_ds(
        ...
        horiz_press_grad allows z_tilde_bot to differ per cell via a
        gradient setting.
        """
        config = self.config
        x = self.x
    ...
    def init_tracers(
        ...
        piecewise node values in the configuration to the current z-tilde
        layer midpoints.
        """
        z_tilde_mid = ds.ZTildeMid
        return self._interpolate_t_s(ds=ds, z_tilde_mid=z_tilde_mid)
    ...
    def _interpolate_t_s(
        ...
        """
        Compute temperature and salinity by PCHIP interpolation from
        piecewise node values defined in the configuration.
        """
        x = self.x
