...
def compute_tracers_linear_in_pressure(
    ...
    * The profile is a fixed point, not a formula.  Temperature is
    ...
    if surf_pressure is None:
        surf_pressure = 0.0
    if not isinstance(surf_pressure, xr.DataArray):
        # pressure_from_geom_thickness() takes a DataArray, so broadcast a
        # uniform surface pressure over the cells
        surf_pressure = xr.full_like(
            layer_thickness.isel(nVertLevels=0), float(surf_pressure)
        )
