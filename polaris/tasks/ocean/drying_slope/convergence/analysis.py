...
class Analysis(ConvergenceAnalysis):
    ...
    def __init__(self, component, subdir, dependencies, damping_coeff):
        """
        Create the step
        ...
        for time in ['0.05', '0.15', '0.25', '0.30', '0.40', '0.50']:
            filename = f'r{damping_coeff}d{time}-analytical.csv'
            self.add_input_file(
                filename=filename, target=filename, database='drying_slope'
            )
    ...
    def exact_solution(self, mesh_name, field_name, time, zidx=None):
        ...
        init = init.drop_vars(
            np.setdiff1d([j for j in init.variables], ['yCell', 'ssh'])
        )
        ...
        day = time / (3600.0 * 24.0)
        datafile = f'./r{self.damping_coeff}d{day:.2f}-analytical.csv'
