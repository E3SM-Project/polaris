...
class Validate(Step):
    ...
    def __init__(self, component, step_subdirs, indir):
        ...
        for subdir in step_subdirs:
            ...
            self.add_input_file(
                filename=f'land_ice_fluxes_{subdir}.nc',
                target=f'../{subdir}/land_ice_fluxes.nc',
            )
            self.add_input_file(
                filename=f'frazil_{subdir}.nc', target=f'../{subdir}/frazil.nc'
            )
    ...
    def run(self):
        ...
        if not land_ice_pass:
            raise ValueError(
                f'Validation failed comparing land ice outputs '
                f'between {step_subdirs[0]} and '
                f'{step_subdirs[1]}.'
            )
        if not frazil_pass:
            raise ValueError(
                f'Validation failed comparing frazil ice outputs '
                f'between {step_subdirs[0]} and '
                f'{step_subdirs[1]}.'
            )
