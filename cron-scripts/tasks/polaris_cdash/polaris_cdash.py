...
def read_tag_file(results_dir):
    ...
    if len(lines) < 2:
        raise ValueError(
            f'TAG file at {tag_path} contains fewer than 2 lines.'
        )
...
def process_build_xml(args, folder_name, build_stamp, sys_info):
    # "there is a folder with the same name to the first line of TAG file.
    # in the folder, there is Build.xml"
    source_build_xml = os.path.join(args.results_dir, folder_name, 'Build.xml')

    if not os.path.exists(source_build_xml):
        print(
            f'Warning: Build.xml not found at {source_build_xml}. Generating '
            f'minimal Build.xml instead.'
        )
        # Fallback or error? User said "Use it instead of generating it."
        # I'll try to generate a minimal one if missing, but primarily we
        # expect it.
        # For now let's error if strictly required, but safer to warn.
        # Actually user instructions imply it exists. I will error if not found
        # to be explicit.
        raise FileNotFoundError(
            f'Source Build.xml not found at {source_build_xml}'
        )
    ...
    site = tree.getroot()
    if site.tag != 'Site':
        # Check if root is Site, sometimes it might be different? XML usually
        # <Site ...>
    ...
    # User didn't fetch system info explicitly for Build.xml, but we used to
    # add it.
    # Let's preserve existing attributes unless we need to overwrite.
    # But usually <Site> has OS info. We can update it if missing or just
    # trust existing.
    # User instruction: "In Build.xml modify [Names]... Also... use [Names]...
    # of Test.xml... to the same data to Build.xml"
    # Doesn't explicitly say "update OS info". I will leave OS info as is from
    # the source file.
...
def generate_test_xml(args, site_attribs, sys_info):
    ...
    # Ensure our CLI args override if not already (process_build_xml updated
    # the attribs, so they should be correct)

    testing = ET.SubElement(site, 'Testing')

    start_time = int(time.time())
    formatted_start_time = time.strftime(
        '%b %d %H:%M %Z', time.localtime(start_time)
    )
    ...
    for log_file in log_files:
        ...
        if 'POLARIS TASK: PASS' not in content:
            ...
            'POLARIS BASELINE:' in content
            and 'POLARIS BASELINE: PASS' not in content
        ):
            status = 'failed'
        else:
            status = 'passed'
    ...
    for test_data in tests:
        ...
        ET.SubElement(
            test_elem, 'FullName'
        ).text = f'./{args.log_dir}/{test_data["name"]}'
        ET.SubElement(
            test_elem, 'FullCommandLine'
        ).text = f'cat {test_data["path"]}'

        results = ET.SubElement(test_elem, 'Results')

        named_meas_time = ET.SubElement(
            results,
            'NamedMeasurement',
            type='numeric/double',
            name='Execution Time',
        ...
        named_meas_status = ET.SubElement(
            results,
            'NamedMeasurement',
            type='text/string',
    ...
    formatted_end_time = time.strftime(
        '%b %d %H:%M %Z', time.localtime(int(time.time()))
    )
...
def main():
    parser = argparse.ArgumentParser(
        description='Generate CDash XML files from log directory'
    )
    ...
    parser.add_argument(
        '--results-dir',
        required=True,
        help='Directory containing TAG file and Build.xml subdirectory',
    )
    # Removed --build-stamp
    ...
    parser.add_argument(
        '--output-dir', default='.', help='Directory to output XML files'
    )
