...
def _make_config(
    ...
    """Return a minimal ConfigParser for PStarInitStep tests.
...
def test_surface_pressure_total_pseudo_thickness():
    ...
    This verifies that the p-star scaling scale = (BP - SP) / BP in
    init_pstar_vertical_coord is applied correctly.  A wrong scaling
    produces pseudo-thicknesses that don't span the right range, which
    corrupts the ZTildeMid values used in tracer interpolation without
    causing an obvious convergence failure.
    """
    target_depth = 500.0
    ssp = RhoSw * Gravity * 1.0
    config = _make_config(rhoref=RhoSw, bottom_depth=target_depth)
...
def test_two_cell_surface_pressure_gradient():
    ...
    exercises the default _build_pstar_coord_ds path with a per-cell surface
    pressure vector.
    """
    target_depth = 500.0
...
def test_full_cell_stagnation_warning_and_early_exit(caplog):
    ...
    class _StagnantStep(_ConstantTracerPStarStep):
        """Override _build_pstar_coord_ds to snap to a fixed pressure."""
        ...
        def _build_pstar_coord_ds(
            ...
            ds = super()._build_pstar_coord_ds(
