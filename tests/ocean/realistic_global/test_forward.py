...
def test_omega_global_stats_are_snapshots_not_reductions():
    ...
    instantaneous sample plain ``<stem>``.  The readers build the plain names
    -- ``global_stats_var_names`` with no time-mean period -- so the analysis
    group has to be configured for snapshots or the variables they look for
    will not exist in the output.
    """
    stage = ForwardStage.from_config(_forward_config())
    ...
    # the names the readers look for are the plain (snapshot) form
    names = global_stats_var_names(
        fields=['temperature'],
        stats=['max'],
        model='omega',
        field_map={'temperature': 'Temperature'},
    )
    assert names[('temperature', 'max')] == 'Temperature_SpatialMax'
...
def test_stats_analysis_fields_come_from_the_forward_config():
    """
    The fields to plot are a forward-run option, so every forward task has
    it.  Left empty, as it is by default, the step plots whatever the run
    ...
    step = StatsAnalysis(
        component=component, indir='WORK', forward_step=_FakeForwardStep()
    )
    step.config = config
    ds = xr.Dataset(
        {
            'temperatureMax': ('Time', [1.0]),
            'temperatureMin': ('Time', [0.0]),
        }
    )
    assert step._fields(ds, 'mpas-ocean') == ['temperature']

    config.set('realistic_global_forward_stats', 'fields', 'salinity')
    assert step._fields(ds, 'mpas-ocean') == ['salinity']
