...
- mask output is mapped back to native ocean names for Omega (e.g.
  `NCells` and `NEdges`); region and transect dimensions and mask variables
  keep their MPAS names until Omega defines its own.
