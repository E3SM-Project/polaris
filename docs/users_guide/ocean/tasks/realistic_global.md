...
- {ref}`ocean-realistic-global-analysis-test`, a one-year Omega run that
  writes what the {ref}`ocean-analysis` tasks read.
...
The `analysis_test` task supports only Omega, since the analysis reads only
Omega output.
...
(ocean-realistic-global-analysis-test)=

## analysis_test

This task runs one simulated year on the `QU.240km` mesh with Omega's
`MonthlyAverages` and `MOC` analysis groups on, writing monthly means of the
fields the {ref}`ocean-analysis` tasks plot, the monthly overturning
streamfunction and daily global statistics.  It is the simulation the
analysis is tested against, and it can be set up with:

```bash
polaris setup -t ocean/spherical/realistic_global/QU.240km/analysis_test/task ...
```

It is the only task in the {ref}`ocean-suite-omega-analysis-test`.

### description

The task has one step, `forward`, which runs Omega from a cached initial
condition for a year.  A year is the shortest run that exercises the whole
analysis chain: the climatology needs whole years to form seasonal and annual
means, and it is the analysis step with the most external dependencies.

The run writes, in the step's work directory:

- `monthly_means_1MonthTimeStats.<year>-<month>.nc`, the monthly means
  from the `MonthlyAverages` group of conservative temperature, absolute
  salinity, pseudo-thickness, specific volume, the layer-midpoint and
  interface elevations, sea surface height, normal velocity and kinetic
  energy;
...
Omega names a time-mean file for the instant it was finalized, the start of
the following month, so the twelve files covering year 1 are named `0001-02`
through `0002-01`.  The analysis accounts for this; see
{ref}`ocean-analysis`.

To analyze the run, point the `omega_analysis` suite at the `omega.yml` in
the `forward` step's work directory:

```cfg
[ocean_analysis]

omega_config_filename = <work_dir>/ocean/spherical/realistic_global/QU.240km/analysis_test/forward/omega.yml
...
The mesh is the quasi-uniform 240-km `QU.240km` E3SM mesh, downloaded from
the Polaris input database rather than generated by the task, as for the
{ref}`ocean-realistic-global-analysis-members` tasks.

### vertical grid

The vertical grid is the one supplied with the cached initial condition, so
there are no vertical-grid config options.

### initial conditions

The initial condition is the cached, TEOS-10 Omega file from the
`realistic_global` section of the Polaris input database, which supplies the
mesh, vertical coordinate and initial state.
...
The number of cores used by the `forward` step is determined algorithmically
from the number of cells in the mesh.
