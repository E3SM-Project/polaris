...
`overflow:` and `overflow:`, defaulting to X by Xkm. 
...
where $z_{shelf}$ corresponds to config option `shelf_depth`, $z_{floor}$ to
`max_bottom_depth`, $x_{slope}$ to `x_slope` and $L_{slope}$ to `l_slope`.
...
shelf region set by the config option `low_temperature` (default value of 10
degC) and high temperature over the slope and deep ocean set by the config
option `high_temperature` (default value of 20 degC). The transition between
the two zones is set by the config option `x_dense` (default value of 20 km).
...
`eos_linear_beta` and `eos_linear_Sref`.
...
config option X. The analysis step is a substitute for the viz step as
